...
Note:
장르의 전체 재생 횟수를 계산하는 countMap과 각 장르에 속한 노래별 재생 횟수를 저장하는 genreMap을 두고
...
class Solution {
    fun solution(genres: Array<String>, plays: IntArray): IntArray {
        ...
        for ((i, genre) in genres.withIndex()) {
            countMap[genre] = countMap.getOrPut(genre) { 0 } + plays[i]
            genreMap.getOrPut(genres[i]) { mutableListOf() }.apply { add(plays[i] to i) }
        }

        val sorted = countMap.toList().sortedByDescending { it.second }

        val answer = mutableListOf<Int>()
        for ((genre, _count) in sorted) {
            val songs = genreMap[genre]!!.sortedByDescending { it.first }.take(2)
            for (song in songs) {
