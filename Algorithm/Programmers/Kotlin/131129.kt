...
Note:
issorb map에는 다트를 던져서 나올 수 있는 모든 점수에 대해 싱글이거나 불로 가능한 점수라면 1을, 아니라면 0을 저장
...
class Solution {
    fun solution(target: Int): IntArray {
        val issorb = mutableMapOf<Int, Int>().apply {
            for (i in 1 .. 20) {
                this[i * 2] = 0
                this[i * 3] = 0
            }
        ...
        val dmin = IntArray(100001) { Int.MAX_VALUE }
        val sorbmax = IntArray(100001)

        for ((score, sorb) in issorb.entries) {
            dmin[score] = 1
            sorbmax[score] = sorb
        }

        for (t in 1 .. target) {
            if (dmin[t] != Int.MAX_VALUE) continue

            var tdmin = Int.MAX_VALUE
            var tsorb = 0
            for ((score, sorb) in issorb.entries) {
                val temp = t - score
                if (temp < 0) continue

                if (dmin[temp] + 1 < tdmin || (dmin[temp] + 1 == tdmin && sorbmax[temp] + sorb > tsorb)) {
                    tdmin = dmin[temp] + 1
                    tsorb = sorbmax[temp] + sorb
                }
            }
        ...
        return intArrayOf(dmin[target], sorbmax[target])
    }
}