...
mapIndexedNotNull을 사용하여 결과가 null이 아닌 요소만 리스트에 포함시킨 다음, IntArray로 변환
...
class Solution {
    fun solution(today: String, terms: Array<String>, privacies: Array<String>): IntArray {
        val termMap = terms.associate { term ->
            val (kind, duration) = term.split(" ")
            kind to duration.toInt()
        }

        val todayDays = changeDays(today)

        return privacies.mapIndexedNotNull { idx, privacy ->
            val (date, term) = privacy.split(" ")
            val expireDays = changeDays(date) + termMap[term]!! * 28

            if (expireDays <= todayDays) idx + 1 else null
        }.toIntArray()
    }

    private fun changeDays(day: String): Int {
        val (yy, mm, dd) = day.split(".").map { it.toInt() }
