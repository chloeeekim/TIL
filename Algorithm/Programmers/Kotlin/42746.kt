...
테스트 케이스 중 numbers가 여러 개의 0으로만 이루어져 있는 경우에 실패하는 듯 하여 결과 문자열이 0으로 시작하면 "0"을, 아니라면 문자열을 리턴

 */

class Solution {
    fun solution(numbers: IntArray): String {
        val answer = numbers.map { it.toString() }.sortedByDescending { it.repeat(3) }.joinToString("")
