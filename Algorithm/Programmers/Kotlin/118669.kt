...
class Solution {
    fun solution(n: Int, paths: Array<IntArray>, gates: IntArray, summits: IntArray): IntArray {
        val map = mutableMapOf<Int, MutableList<Pair<Int, Int>>>().apply {
            for ((i, j, w) in paths) {
                this.getOrPut(i) { mutableListOf() }.add(j to w)
                this.getOrPut(j) { mutableListOf() }.add(i to w)
            }
        }

        val summitSet = summits.toSet()
        val visited = IntArray(n + 1) { Int.MAX_VALUE }
        val heap = PriorityQueue<Pair<Int, Int>>({ a, b -> a.first - b.first }).apply {
            for (g in gates) {
                add(0 to g)
                visited[g] = 0
            }
        ...
        while (heap.isNotEmpty()) {
            val (intensity, curr) = heap.poll()

            if (curr in summitSet || visited[curr] < intensity) continue

            for ((j, w) in map[curr]!!) {
                val temp = if (intensity > w) intensity else w
                if (temp < visited[j]) {
                    visited[j] = temp
                    heap.add(temp to j)
                }
