...
구매 가능한 품목과 원하는 품목의 개수를 비교하여 모두 만족하면(all) answer++

 */

class Solution {
    fun solution(want: Array<String>, number: IntArray, discount: Array<String>): Int {
        val window = discount.slice(0 until 10).groupingBy { it }.eachCount().toMutableMap()

        var answer = 0
        for (i in 0 .. discount.size - 10) {
            if (want.indices.all { window[want[it]] == number[it] }) answer++

            if (i + 10 < discount.size) {
                val remove = discount[i]
                val add = discount[i+10]

                window[add] = (window[add] ?: 0) + 1
                val removeCount = window[remove]!! - 1
                if (removeCount == 0) window.remove(remove)
                else window[remove] = removeCount
            }
        }
