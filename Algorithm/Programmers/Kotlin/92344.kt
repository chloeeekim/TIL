...
Note:
효율성 테스트에서 시간 초과를 막기 위해 누적합(prefix sum)을 이용하여 해결
operator fun으로 component1부터 component6까지 정의하여 destructuring 문제 해결
...
class Solution {
    fun solution(board: Array<IntArray>, skill: Array<IntArray>): Int {
        ...
        val sum = Array(height + 1) { IntArray(width + 1) }

        for ((type, r1, c1, r2, c2, degree) in skill) {
            sum[r1][c1] += if (type == 2) degree else -degree
            sum[r1][c2 + 1] -= if (type == 2) degree else -degree
            sum[r2 + 1][c1] -= if (type == 2) degree else -degree
            sum[r2 + 1][c2 + 1] += if (type == 2) degree else -degree
        }
