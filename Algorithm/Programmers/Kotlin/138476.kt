...
groupingBy와 eachCount를 사용하여 동일한 무게를 지닌 귤들의 개수를 카운트
귤의 개수에 대해 내림차순으로 정렬하여 많은 것부터 더해가는 방식으로, greedy하게 해결
...
class Solution {
    fun solution(k: Int, tangerine: IntArray): Int {
        ...
        val tCount = tangerine.toTypedArray().groupingBy { it }.eachCount()
            .values.sortedDescending()

        var total = 0
        for ((idx, num) in tCount.withIndex()) {
            total += num
            if (total >= k) return idx + 1
        }
