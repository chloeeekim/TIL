...
class Solution {
    fun solution(numbers: LongArray): IntArray {
        return numbers.map { num ->
            val binary = getFullTree(num.toString(2))
            if (checkValidTree(binary)) 1 else 0
        }.toIntArray()
    }

    private fun checkValidTree(tree: String): Boolean {
        if (tree == "1" || tree == "0") return true

        val root = tree.length / 2
        if (tree[root] == '0' && '1' in tree) return false

        return checkValidTree(tree.substring(0, root)) && checkValidTree(tree.substring(root + 1, tree.length))
    }

    private fun getFullTree(binary: String): String {
        val l = binary.length
        var count = 1
        var level = 1
