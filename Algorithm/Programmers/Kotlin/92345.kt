...
class Solution {
    fun solution(board: Array<IntArray>, aloc: IntArray, bloc: IntArray): Int {
        ...
        fun solve(ax: Int, ay: Int, bx: Int, by: Int): Int {
            ...
            for ((dx, dy) in move) {
                ...
                if (nx !in (0 until height) || ny !in (0 until width) || board[nx][ny] == 0) continue

                board[ax][ay] = 0
                val temp = solve(bx, by, nx, ny) + 1
                board[ax][ay] = 1

                if (temp % 2 == 1 && res % 2 == 0) res = temp
                else if (temp % 2 == 0 && res % 2 == 0) res = maxOf(res, temp)
                else if (temp % 2 == 1 && res % 2 == 1) res = minOf(res, temp)
            }
