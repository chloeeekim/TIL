...
Note:
최종적으로 변경된 닉네임을 확인하기 위해 map 사용
닉네임 변경에 대해서는 결과 메시지를 출력하지 않으므로, 채팅방 입장 및 퇴장에 대해서만 결과에 추가

 */

class Solution {
    fun solution(record: Array<String>): Array<String> {
        val message = mapOf("Enter" to "님이 들어왔습니다.", "Leave" to "님이 나갔습니다.")
        ...
        for (r in record) {
            ...
            when (sr[0]) {
                "Enter" -> {
                    nickname[sr[1]] = sr[2]
                    result.add(sr[0] to sr[1])
                }
                "Leave" -> result.add(sr[0] to sr[1])
                "Change" -> nickname[sr[1]] = sr[2]
            }
        }

        return result.map { (op, uid) ->
            nickname[uid] + message[op]
        }.toTypedArray()
    }
}