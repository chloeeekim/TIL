...
class Solution {
    fun solution(diffs: IntArray, times: IntArray, limit: Long): Int {
        var left = 1
        var right = diffs.maxOrNull() ?: 0

        while (left < right) {
            val mid = (left + right) / 2
            val duration = calcDuration(diffs, times, mid)

            if (duration > limit) left = mid + 1
            else right = mid
        }
    ...
    private fun calcDuration(diffs: IntArray, times: IntArray, level: Int): Long {
        var total = 0L
        ...
        for (i in diffs.indices) {
            ...
            if (diff <= level) total += time.toLong()
            else {
                val tries = (diff - level).toLong()
                total += times[i - 1] * tries + time * (tries + 1)
            }
        }
