...
PriorityQueue를 사용하여 우선순위에 따른 대기 큐를 구현
...
class Solution {
    data class Job(val idx: Int, val request: Int, val time: Int)

    fun solution(jobs: Array<IntArray>): Int {
        val heap = PriorityQueue<Job>(compareBy<Job> { it.time }.thenBy { it.request }.thenBy { it.idx })
        var curr = 0
        var finished = 0
        var answer = 0
        var before = -1
        ...
        while (finished < jobs.size) {
            for ((idx, job) in jobs.withIndex()) {
                if (before < job[0] && job[0] <= curr) {
                    heap.add(Job(idx, job[0], job[1]))
                }
            }

            if (heap.isNotEmpty()) {
                val (idx, request, time) = heap.poll()
                before = curr
                curr += time
                answer += curr - request
                finished++
