...
def solution(n, tops):
    mod = 10007
    type_a, type_b = [0] * n, [0] * n
    type_a[0] = 1
    type_b[0] = 3 if tops[0] == 1 else 2

    for i in range(1, n):
        if tops[i] == 1:
            type_a[i] = type_a[i-1] + type_b[i-1]
            type_b[i] = type_a[i-1] * 2 + type_b[i-1] * 3
        else:
            type_a[i] = type_a[i-1] + type_b[i-1]
            type_b[i] = type_a[i-1] + type_b[i-1] * 2

        type_a[i] %= mod
        type_b[i] %= mod

    return (type_a[-1] + type_b[-1]) % mod