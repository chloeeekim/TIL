...
def solution(plans):
    ...
    plans = [[a, toMinutes(b), int(c)] for a, b, c in plans]
    plans = sorted(plans, key=lambda x: x[1])

    for i in range(len(plans)):
        name, start, playtime = plans[i]
        temp = plans[i+1][1] - start if i != len(plans) - 1 else 9999999
        ...
        if temp >= playtime:
            ...
            while stack:
                ...
                if temp >= remain:
                    ...
                    stack.append((name, remain - temp))
                    break
        else:
            stack.append((name, playtime - temp))
